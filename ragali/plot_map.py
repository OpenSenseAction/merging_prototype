...
def scatter_line_v2(xarray_obj, color='k', linewidth=2, pad_width=1, ax=None):
    if ax is None:
        fig, ax = plt.subplots()
    else:
        fig = ax.get_figure()

    x0 = np.atleast_1d(xarray_obj.site_0_lon.values)
    y0 = np.atleast_1d(xarray_obj.site_0_lat.values)
    x1 = np.atleast_1d(xarray_obj.site_1_lon.values)
    y1 = np.atleast_1d(xarray_obj.site_1_lat.values)
